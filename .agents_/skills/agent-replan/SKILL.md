...
Перепланировать задачу в уже созданном PR: обновить артефакты плана (`tasks/T-XXXX.md`, `spec.md`) и вернуть PR в состояние ожидания апрува плана.
...
```
Обязательно прочитай комментарии в PR, чтобы понять причину реплана (например, замечания ревьюера или изменения требований).

### 2) Update plan artifacts

Обновить:

- `tasks/T-<issue_number>.md` (шаги/Goal/Acceptance). Сохраняй строгую структуру (Why, Goal, Plan, Acceptance).
- `spec.md` (уточнить/добавить `R-XXX`), если изменились требования к поведению системы.
