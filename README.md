...
Технический долг, рефакторинг и изменения инфраструктуры оформляются как `chore(area): …` и проходят ту же цепочку Plan Gate.

В PR body для таких задач поле `Spec` заполняется `n/a` (или `Internal change`).
Это сигнал, что поведение **не изменяется**, поэтому спецификация не применима. При
этом ответственность за объяснение «зачем» лежит в поле `Why`.

* Первый коммит PR — **план-only** с файлом `tasks/T-XXXX.md`. План должен кратко описывать задачу и способ доказать, что поведение **не изменилось** (обычно прогон существующих тестов).
* В PR body указываются:
  * `Fixes: #<номер>`
  * `Spec: n/a` (или `Internal change`)
  * `Plan: tasks/T-XXXX.md`
  * `Why:` техническая мотивация (2–5 строк)
* После апрува плана (`plan:approved`) переводите issue в `status:todo`/`in-progress` и пушите реализацию.

Это помогает сохранять документацию и контроль, даже когда поведение не меняется.
