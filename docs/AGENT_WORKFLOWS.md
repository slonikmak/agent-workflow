...
* `workflow`: Метка для задач по развитию самого автоматизированного процесса. Позволяет отделить мета-задачи от продуктовой разработки в отчетах.
...
4.  **`report`**: Вывести текущее состояние очередей (backlog, todo, in-progress, open PRs). Задачи с меткой `workflow` учитываются отдельно для отслеживания доработок самого процесса.
